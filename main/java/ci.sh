...
function helm_build(){
     mkdir -p deploy
	 cp -rf /root/deploy/file/java/deploy/* deploy
	 if [  -d "k8s" ];then
	 echo "存在自定义k8s 文件"
	 cp -rf k8s/* deploy/templates
	 fi
     cp -f /root/deploy/file/${Namespace}/${ServiceName}/values-${1}.yaml deploy/values.yaml
...
docker_build(){
    ...
    if [  -f "Dockerfile" ];then
        ...
        cp -rf /root/deploy/file/java/docker/* .
    fi
